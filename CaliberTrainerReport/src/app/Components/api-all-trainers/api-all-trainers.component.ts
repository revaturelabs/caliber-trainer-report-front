...
export class ApiAllTrainersComponent implements OnInit {
  ...
  constructor(
    ...
    private as: GetAssessmentService,
                  ...
                  await this.qcs.getPromiseQCNotesByBatchId(id).then(
                    async (response3) => {
                      ...
                      for (let note of tempNotes) {
                        await this.qcs
                          .getCategoryByBatchIdAndWeek(id, note.week)
                          .toPromise()
                          .then(
                            async (response4) => {
                              tempNote = note;
                              ...
                              if (cat != null) {
                                for (let c of cat) {
                                  //For each category, add the category to the array found in the batch
                                  if (
                                    c != null &&
                                    !tempNote.categories.includes(
                                      c.skillCategory
                                    )
                                  ) {
                                    tempNote.categories.push(c.skillCategory);
                                    console.log(
                                      'temp note: ' + JSON.stringify(tempNote)
                                    );
                                  }
                                }
                              }
                      ...
                      for (let i = 0; i < batch.currentWeek; i++) {
                        await this.as
                          .getPromiseAssessmentsByWeekAndBatchId(id, i + 1)
                          .then(
                            // Works
                            async (response5) => {
                              ...
                              for (let j = 0; j < assessments.length; j++) {
                                await this.cs
                                  .getPromiseCategoryById(
                                    assessments[j].assessmentCategory
                                  )
                                  .then(
                                    // Works
                                    async (response6) => {
                                      //console.log("skill category");
                                      assessments[j].skillCategory =
                                        response6.skillCategory;
                                      //console.log(assessments[i].skillCategory);

                                      await this.as
                                        .getAverageGradeByAssessment(
                                          assessments[j].assessmentId
                                        )
                                        .toPromise()
                                        .then(
