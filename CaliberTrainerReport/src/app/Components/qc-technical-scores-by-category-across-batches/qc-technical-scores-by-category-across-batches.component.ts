...
export class QcTechnicalScoresByCategoryAcrossBatchesComponent
  ...
  updateGraph() {
    ...
    let gA2: any[] = JSON.parse(
      sessionStorage.getItem('graphingArray2' + trainerId)
    );
  ...
  displayGraph(batchDisplayNames: string[], yDisplayValues: any[]) {
    ...
    if (this.pickedCategory == 0) {
      ...
      for (let i = 1; i < this.categoriesName.length; i++) {
        if (
          this.multiGraphYValues[i - 1].reduce(batchRemoveEmptyReduce, 0) !== 0
        ) {
          ...
          let dataObj = {
            ...
            data: this.batchFilter.filterBatch(
              this.multiGraphYValues[i - 1],
              this.batchFlags
