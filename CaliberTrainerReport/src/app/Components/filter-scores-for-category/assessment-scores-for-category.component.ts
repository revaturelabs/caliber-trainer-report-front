...
@Component({
  ...
  templateUrl: './assessment-scores-for-category.component.html',
  styleUrls: ['./assessment-scores-for-category.component.css'],
})
export class AssessmentScoresForCategoryComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.assessmentServiceSubscription = this.assessmentService
      ...
      .subscribe(
        (resp) => {
          for (const score of resp.categories) {
            if (!this.isCategoryEmpty(score.batchAssessments)) {
              ...
              this.multiGraphYValues.push(
                JSON.parse(
                  JSON.stringify(this.getBatchAverages(score.batchAssessments))
                )
              );
        ...
        () => {
          console.log(
            'An error has occurred building the assessment-scores-for-category.'
          );
        }
      );
  }
  ...
  displayGraph(batchNames: string[], yValues: number[]) {
    ...
    if (this.pickedCategory == 0) {
      ...
      let lineColor: string =
        colorArray[(this.pickedCategory - 1) % colorArray.length];
      var i;
      console.log(yValues);
  ...
  private isCategoryEmpty(
    batchAssessments: { assessments: number[] }[]
  ): boolean {
  ...
  private getBatchAverages(
    ...
    for (const assessments of batchAssessments) {
      if (!assessments.assessments || assessments.assessments.length == 0) {
        ...
        yValueSet.push(
          assessments.assessments.reduce((acc, curr) => acc + curr) /
  ...
  updateGraph() {
    if (this.pickedCategory == 0) {
      this.displayGraph(
        this.batchFilter.filterBatch(this.batchNames, this.batchFlags),
        this.batchFilter.filterBatch(this.multiGraphYValues, this.batchFlags)
      );
    } else {
      this.displayGraph(
        this.batchFilter.filterBatch(this.batchNames, this.batchFlags),
        this.batchFilter.filterBatch(
          this.multiGraphYValues[this.pickedCategory - 1],
          this.batchFlags
        )
      );
    }
